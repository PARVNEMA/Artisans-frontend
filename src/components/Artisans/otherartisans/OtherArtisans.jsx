...
function OtherArtisans() {
  ...
  useEffect(() => {
    ...
    getArtisansProduct();
  }, [getCurrentArtisans, getArtisansSellerMetrices, getArtisansProduct]);

  const metricColors = {
    customerSatisfaction: "text-blue-500",
    disputeRate: "text-green-500",
    productSellingRate: "text-purple-500",
    refundRate: "text-red-500",
    returnRate: "text-yellow-500",
  };

  return (
    <div>
      <div className="text-black flex flex-col mx-[2rem] md:mx-[8rem] my-5 gap-[3rem]">
        ...
        <div className="flex w-full flex-col md:flex-row justify-between">
          ...
          <img
            className="w-[14rem] h-[14rem] rounded-full mt-4 md:mt-0"
            src={artisans?.avatar}
            alt=""
          />
        </div>
        <div className="mt-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 items-center justify-between bg-white p-6 rounded-lg shadow-lg">
            <div className="flex flex-col items-center">
              <DonutChart
                value={artisansmatrices?.customerSatisfaction}
                color={metricColors.customerSatisfaction}
              />
              <p className="mt-4 font-bold">Customer Satisfaction</p>
            </div>
            <div className="flex flex-col items-center border-t md:border-t-0 md:border-l border-gray-300 pt-6 md:pt-0 md:pl-6">
              <DonutChart
                value={artisansmatrices?.disputeRate}
                color={metricColors.disputeRate}
              />
              <p className="mt-4 font-bold">Dispute Rate</p>
            </div>
            <div className="flex flex-col items-center border-t md:border-t-0 md:border-l border-gray-300 pt-6 md:pt-0 md:pl-6">
              <DonutChart
                value={artisansmatrices?.productSellingRate}
                color={metricColors.productSellingRate}
              />
              <p className="mt-4 font-bold">Sales Rate</p>
            </div>
            <div className="flex flex-col items-center border-t md:border-t-0 md:border-l border-gray-300 pt-6 md:pt-0 md:pl-6">
              <DonutChart
                value={artisansmatrices?.refundRate}
                color={metricColors.refundRate}
              />
              <p className="mt-4 font-bold">Refund Rate</p>
            </div>
            <div className="flex flex-col items-center border-t md:border-t-0 md:border-l border-gray-300 pt-6 md:pt-0 md:pl-6">
              <DonutChart
                value={artisansmatrices?.returnRate}
                color={metricColors.returnRate}
              />
      ...
      <div className="mx-[2rem] md:mx-[10rem] text-start">
