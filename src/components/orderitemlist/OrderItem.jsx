...
function OrderItem() {
  ...
  const onFileChange1 = (e) => setSelectedFiles1(e.target.files);
  const onFileChange2 = (e) => setSelectedFiles2(e.target.files);

  const onSubmit = async (data, orderItemId) => {
    if (!selectedFiles1.length || !selectedFiles2.length) {
      toast.error("Please select both pickup and delivery images.");
      return;
    }
  ...
  return (
    <div>
      ...
      {Array.isArray(ordersitems) && ordersitems.length > 0 ? (
        ordersitems.map((item, index) => (
          <div key={item.orderItemId} className="mb-6 border p-4 rounded-lg">
            <h2 className="text-xl font-bold mb-2">Delivery Address</h2>
            <p>
              {item.deliveryAddress.city}, {item.deliveryAddress.state},{" "}
              {item.deliveryAddress.country} - {item.deliveryAddress.zipCode}
            </p>

            <h2 className="text-xl font-bold mt-4 mb-2">Pickup Address</h2>
            <p>
              {item.pickupAddress.city}, {item.pickupAddress.state},{" "}
              {item.pickupAddress.country} - {item.pickupAddress.zipCode}
            </p>
            ...
            >
              ...
              <div className="flex flex-col mb-4">
                <label htmlFor={`status-${index}`} className="font-bold mb-1">
                  Change Status
                </label>
                <select
                  name="status"
